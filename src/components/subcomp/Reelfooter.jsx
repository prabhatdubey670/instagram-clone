...
function Reelfooter({username,caption,music}) {
  return (
    <div id="reelfooter" className='relative bottom-[77px] px-2 bg-transparent text-white flex flex-col'>
      ...
      <p className='text-sm
      absolute left-2 -bottom-[44px] bg-transparent'>{caption} </p>
      <BiMusic className='absolute -bottom-16 bg-transparent font-bold z-50'/>
       <motion.p
        initial={{ x: -330,opacity:0.2 }}  animate={{ x: 270,opacity:1 }} transition={{ repeat: Infinity,type:"keyframes", duration: 3,repeatType:'loop' }}
